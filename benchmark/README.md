...
## V0.4 Skill dependency static pilot

[`annotations/v0.4-skill-dependency-pilot.json`](annotations/v0.4-skill-dependency-pilot.json) records five static boundary samples from the existing pinned corpus: two conditional references and three negative/generic references. It validates evidence conventions and false-positive boundaries only. It does not modify ground truth, claim recall/precision, or run a model. The pinned corpus does not contain enough explicit versioned/transitive/cyclic Skill dependencies for a formal V0.4 benchmark; deterministic graph behavior is covered by unit tests and 12 synthetic evals.

The benchmark contracts are prepared for Handoff 1.2 and exact namespace-qualified Skill identities. When a separately approved and reviewed V0.4 corpus exists, the scorer can report Skill recall/precision, required-Skill recall, Skill false ready, Inventory coverage, and Skill UNKNOWN rate. These fields remain `N/A` for the current V0.1–V0.3 ground truth and must not be presented as measured V0.4 accuracy.
